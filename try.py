...
def main():
    ...
    input_data={
	"main": [{
			"bucket_name": "ans-marketing",
			"filename": "sorting/Jack & Jones.csv"
			
		    }]
            }
    
    for entry in input_data['main']:
        bucket_name = entry.get('bucket_name')
        filename = entry.get('filename')
        print("Bucket Name:", bucket_name)
        print("Filename:", filename)
    
        try:
            df=bucket.read_csv(f"{bucket_name}", f"{filename}")
            print(df)
            print("Done")

            # Add column names (headers) to Google Sheet
...
#           
        ...
        print("DataFrame has been pushed to Google Sheets!")
